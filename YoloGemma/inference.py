...
def _load_model(checkpoint_path, device, precision, use_tp):
    ...
    if "int8" in str(checkpoint_path):
        ...
        from YoloGemma.utils.quantize import WeightOnlyInt8QuantHandler
    ...
    if "int4" in str(checkpoint_path):
        ...
        from YoloGemma.utils.quantize import WeightOnlyInt4QuantHandler
    ...
    if use_tp:
        from YoloGemma.utils.tp import apply_tp
...
def main(
    ...
    global print
    from YoloGemma.utils.tp import maybe_init_dist
    ...
    for i, frame in enumerate(frames):
        ...
        if i % 2== 0:
            ...
            from YoloGemma.utils.parser import extract_objs
            
            
            last_prompt =' '.join(prompt.split()[1:])
            print(processor.decode(y, skip_special_tokens=True))
            if ';' not in decoded_output and ('loc' in decoded_output):
                # detected_objects = extract_objs(text=decoded_output)
                # print(f"""\n\n{detected_objects}\n\n""")
                # locations = detected_objects
                try:
                    locations = [int(loc.replace('loc', '').replace('<', '').replace('>', '').replace(f'{prompt}\n', '').replace(f'{last_prompt}', '')) for loc in decoded_output.split("><") if 'loc' in loc]
                except:
                    locations = []
            else:
                locations = []
